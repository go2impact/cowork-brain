...
└──────────────────────────┬──────────────────────────────────┘
  ...
  └──────┬───────┘         │
         ...
         ▼                 ▼
┌─────────────────────────────────────────────────────────────┐
│              Activity Store (SQLite via libsql)                │
...
- **Vector store:** libsql built-in vector search (same .db as activity store — single file, zero-ops, no extension needed)
...
3. Vector similarity search against libsql's built-in vector store finds the most relevant past messages, activity data, and observations.
...
- **Embeddings / vector store** — libsql built-in vector search (same .db file). Embeddings generated by Qwen3-Embedding-0.6B, stored locally, searched locally. Nothing leaves the device.
- **Working memory profile** — SQLite (structured data, not vectors).
- **Observational memory logs** — SQLite (compressed summaries) + libsql vector search (embedded for semantic search).
...
12. Qwen3-Embedding-0.6B + libsql vector search RAG
